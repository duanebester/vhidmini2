...
HID_REPORT_DESCRIPTOR G_GamepadReportDescriptor[] =
{
	...
	LogicalMaximum(0xff),
	PhysicalMinimum(0),
	PhysicalMaximum(0xff),
	Usage(USB_HID_Y),

	//
	// 2 8-bit absolute values.
	//
	ReportSize(8),
	ReportCount(2),
...
HID_DESCRIPTOR              G_DefaultHidDescriptor = {
    ...
    0x02,   // number of HID class descriptors
    {                                       //DescriptorList[0]
        0x22,                               //report descriptor type 0x22
        sizeof(G_GamepadReportDescriptor)   //total length of report descriptor
    }
};
...
--*/
{
    ...
    //
    if (!NT_SUCCESS(status)){
        deviceContext->ReportDescriptor = G_GamepadReportDescriptor; // DUANE
...
--*/
{
	...
	memset(queueContext->OutputReport, 0, sizeof(queueContext->OutputReport));
...

{
	...
	memcpy(QueueContext->DeviceContext->DeviceData, outputReport->Data, sizeof(outputReport->Data));
...
--*/
{
	...
	reportBuffer->Data[0] = (UCHAR)127;
	reportBuffer->Data[1] = (UCHAR)127;
	reportBuffer->Data[2] = (UCHAR)0;
...
--*/
{
	...
	memcpy(QueueContext->OutputReport, reportBuffer->Data, sizeof(reportBuffer->Data));
...
--*/
{
    ...
    if (NT_SUCCESS(status)) {
		...
		readKBReport.Data[1] = (UCHAR)(0x00);
		readKBReport.Data[2] = (UCHAR)(0x00);
		readKBReport.Data[3] = (UCHAR)(0x00);
		readKBReport.Data[4] = (UCHAR)(0x00);
		readKBReport.Data[5] = (UCHAR)(0x00);
